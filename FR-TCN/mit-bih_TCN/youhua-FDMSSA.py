...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.00530320308):
        """
        相当于一个Residual block
...
class TCN(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.00530320308):#tcn(num_inputs=5,num_channels=[32,16,4,1],kernel_size=3, dropout=0.3)
        """
...
def main():
    ...
    best_score, best_params = MSSA(pop, dim, lb, ub, Max_iter, evaluate_model)
    # print('Best parameters found: lr = {}, batch_size = {}, dropout = {}'.format(best_params[0], int(best_params[1]),
    #                                                         best_params[2]))
    print('FDMSSA end:',best_score, best_params)
