...
'''麻雀发现者更新'''
def PDUpdate(X,PDNumber,ST,Max_iter,dim,t, v, Xold):
    X_new  = copy.copy(X)
    R2 = random.random()
        ...
        if t<6 :
            ...
            if R2 < ST:
                ...
                X_new[j, :] = (v*X[j, :]-0.5*v*(v-1)*X1[j, :]-v*(v-1)*(v-2)*X2[j, :]/6 + v*(v-1)*(v-2)*(v-3)*X3[j, :]/24
                              ...
                              v*(v-1)*(v-2)*(v-3)*(v-4)*(v-5)*(v-6)*X6[j, :]/5040) * (2 / np.exp(4 * t / (np.random.random() * Max_iter) ** 2))
            else:
                X_new[j, :] =(v*X[j, :]-0.5*v*(v-1)*X1[j, :]-v*(v-1)*(v-2)*X2[j, :]/6 + v*(v-1)*(v-2)*(v-3)*X3[j, :]/24
                              + v*(v-1)*(v-2)*(v-3)*(v-4)*X4[j, :]/120 + v*(v-1)*(v-2)*(v-3)*(v-4)*(v-5)*X5[j, :]/720 +
...
'''改进麻雀搜索算法'''
def AMSSA(pop,dim,lb,ub,Max_iter,evaluate_model):
    ST = 0.6 #预警值
    ...
    for i in range(Max_iter):
        ...
        f_threshold = -700
        f = max(f, f_threshold)# 设置下界防止过小
        v = 1 / (2 * np.exp(-0.47 * f))

        X = PDUpdate(X, PDNumber, ST, Max_iter, dim, i, v, Xold)  # 发现者更新
